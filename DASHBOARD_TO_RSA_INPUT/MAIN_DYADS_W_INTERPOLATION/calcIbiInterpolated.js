...
module.exports = (data, samplingRate) => {
    ...
    // make output data
    const ibi_samples = ibi_merged.map(el => el[1]);
    const ibi_ms = ibi_samples.map(samples => samples / samplingRate * 1000);
    const interpolated_indices = ibi_interpolated.map(el => el[0]);
